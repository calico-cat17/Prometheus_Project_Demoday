...
START_BUTTON_RECT = pygame.Rect(440, 545, 240, 58)
...
CLUES = {
    ...
    "performance_sheet": {
        "name": "성과급 평가표",
        "location": "원장실",
        "desc": "성과급 기준은 전국 모의고사 평균, 정예반 유지율, 우수 성적자 비율이다. 기초반 강사에게 불리하다.",
        "tags": ["홍지연 동기처럼 보임"],
    },
    "consult_schedule": {
        "name": "상담 예약표",
...
def draw_dialogue_window() -> None:
    ...
    draw_text(
        screen,
        state.input_text or placeholder,
        (324, 620),
        FONT_SM,
        COLORS["text"] if state.input_text else COLORS["muted"],
    )
